...
export class LineMasterService {
  ...
  private apiUrl = `${environment.apiUrl}/linemaster`;

  isLoading = signal(false);

  public lineMasterList = new BehaviorSubject<any[]>([]);
  public lineMaster = new BehaviorSubject<any>(undefined);
  ...
  removeItem(list: any) {
    let currentList = this.lineMasterList.value;
    let index = currentList.findIndex((item: any) => item._id === list._id);
    if (index > -1) {
      currentList.splice(index, 1);
      this.lineMasterList.next([...currentList]);
    }
  }
}
